...
(define (parse q)
  (cond
    ...
    [(and (list? q)
         (eq? (length q) 3)
          (eq? (car q) 'apply))
    (parse-apply (parse (second q)) (parse (third q)))]
    [(and (list? q)
          (pair? q)
          (not (op->proc (car q)))) ; <------- !!!
     (parse-app q)]
...
(define (parse-apply lambersy argersy)
  (cond [(and (not (lam? (lam-e lambersy))) 
              (equal? (eval (cdr-expr  argersy)) '()))
         (app lambersy (const (eval (car-expr argersy))))]
        [(and (not (lam? (lam-e lambersy)))
              (not (equal? (eval (cdr-expr  argersy)) '())))
         (error "Too many arguments")]
        [(and (lam? (lam-e lambersy))
              (equal? (eval (cdr-expr  argersy)) '()))
         (error "To w sumien nie error")]
        [(and (lam? (lam-e lambersy))
              (not (equal? (eval (cdr-expr argersy)) '())))
         (app (lam (lam-id lambersy) (parse-apply (lam-e lambersy) (cdr-expr argersy))) (const (eval (car-expr argersy))))]))

(define (parse-lam var e) ; <------------------------------------- !!!
  (cond [(= 1 (length var))
         (lam (car var) (parse e))]
        [else
         (lam (car var) (parse-lam (cdr var) e))]))
...
;(eval (parse '(apply (lambda (x y z) (+ x (+ y z))) (list 1 2 3 4))))
(eval (parse '(apply (lambda (x y) (+ x y)) (list 1))))
                              
;(first '((lambda (x y z) (+ x (+ y z))) 1 2 3))


;;(app (lam 'x (app (lam 'y (binop '+ (var-expr 'x) (var-expr 'y))) 2)) 1)
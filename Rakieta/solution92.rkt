...
(define (env-update! x v xss)
  (define (assoc-update! xs)
    (cond [(null? xs) (error "Unknown identifier" x)]
          [(eq? x (mcar (car xs))) (begin (set-mcdr! (car xs) v) xss)]
          [else (env-update! x v (cdr xs))]))
  (assoc-update! (environ-xs xss)))
...
(define (fact1)
  (let* ([final-env
           (eval-while (parse-while WHILE_1) env-empty)])
...
(define (fact2)
  (let* ([final-env
           (eval-while (parse-while WHILE_2) env-empty)])
    (env-lookup 'z final-env)))
(fact2)
...
(define (fact3)
  (let* ([final-env
           (eval-while (parse-while WHILE_3) env-empty)])
    (env-lookup 'z final-env)))
(fact3)
(define WHILE_4
  '{(x := 5)
...
(define (fact4)
  (let* ([final-env
           (eval-while (parse-while WHILE_4) env-empty)])
    (env-lookup 'z final-env)))
(fact4)

;zrobione w kooperacji z Kacper Gawdziński i Kacper Bajkiewicz
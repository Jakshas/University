#lang racket
(provide annotate-expression expr? make-pos parse expr-annot? env-empty env-add env-lookup)
...
(define (annotate-expression e)
  (define (expr e env)
    (match e
      [(const n)  (const n)]
      [(binop op l r) (binop op (expr l env) (expr r env))]
      [(var-expr x)   (if (eq? (env-lookup x env)#f)
                        (var-free x)
                        (var-bound (env-lookup x env) x))]
      [(let-expr loc x e1 e2)
       (let-expr loc x (expr e1 env) (expr e2 (env-add x (make-pos #'x) env)))]
      [_ false]))
  (expr e env-empty)
  )

...
(define effect-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         ...
         `((choose ,(effect-builtin/1 (lambda (x)   x)))
           (fail   ,(effect-builtin/1 (lambda (x)   (choose null))))
           )))
...
(define HETMANY
    '(letrec [vhor (lambda (r board)
                        (if (null? board)
                            true
                            (if (= r (car board))
                                false
                                (vhor r (cdr board)))))]
     (letrec [vdiag (lambda (r board)
                        (if (null? board)
                            true
                            (if (= r (car board))
                                false
                                (and (vdiag (- r 1) (cdr board))
                                     (vdiag (+ r 1) (cdr board))))))]
     (letrec [validate (lambda (r board)
                            (or (null? board)
                                (and  (vhor r board) (vdiag r board))))]
     (letrec [hetmany (lambda (n board)
                (if (= n 9)  
                    board
                    (let [r (choose (list 1 2 3 4 5 6 7 8))]
                         (if (validate r board)
                             (hetmany (+ n 1) (cons r board))
                             (fail false)))))]

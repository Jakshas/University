...
(define (effect-builtin/2 p)
  ...
  (define (assoc-put x n o)
  (define (aux x n xs)
    (if (null? xs)
        (mcons (cons x n) o)
        (if (eq? x (car (mcar xs)))
            (begin 
                (mcar-set! xs (cons x n))
                o)
            (aux x n (mcdr xs)))))
    (aux x n o))

(define (seach-var x xs)
    (if (null? xs)
        (error)
        (if (eq? x (car (mcar xs)))
            (cdr (mcar xs))
            (search-var x (mcdr xs)))))

(define effect-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         start-env
         `((put ,(effect-builtin/2 (lambda (x n) (lambda (o) (cons false (assoc-put x n o)))))) ; 
           (get ,(effect-builtin/1 (lambda (x) (lambda (s)  (cons (search-var x s) s)))))
           )))
...
(const n) oblicza się do wartości n

(binop op l r) oblicza oba swoje argumenty do wartości i oblicza się do wartości

(let-expr x e1 e2) oblicza e1 do wartości, a potem oblicza się do tego, do czego oblicza się e2 (niezależnie czy jest to wartość czy odroczenie)

(var-expr x) oblicza się do tego, co przypisane jest do x w środowisku (niezależnie czy jest to wartość czy odroczenie)

(if-expr b t f) oblicza b do wartości, a następnie oblicza się do tego, do czego oblicza się odpowiednio t lub f (niezależnie czy jest to wartość czy odroczenie)

(cons-expr e1 e2) oblicza e1 oraz e2 do odroczeń, oblicza się do wartości

(car-expr e) oblicza e do wartości, oblicza się do tego, do czego oblicza się pierwszy element pary (a zawsze jest odroczeniem)

(cdr-expr e) jak wyżej, ale drugi element pary

(null-expr) oblicza się do wartości

(null?-expr e) oblicza e do wartości i oblicza się do wartości

(lam x e) oblicza się do wartości

(app f e) oblicza f do wartości, e do odroczenia i oblicza się do tego, do czego oblicza się procedura do której obliczył się f (niezależnie czy jest to wartość czy odroczenie)

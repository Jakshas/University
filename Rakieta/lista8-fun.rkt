...
(struct let-lazy   (id e1 e2)  #:transparent)
(struct unfold     (seed step) #:transparent)
(struct scar       (unf)       #:transparent)
(struct scdr       (unf)       #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(unfold seed step) (and (expr? seed) (expr? step))]
    [(scar unf) (expr? unf)]
    [(scdr unf) (expr? unf)]
    [_ false]))
...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x) (match (env-lookup x env)
                    [(clo id e1 env) (eval-env e1 env)]
                    [v v])]
    [(let-lazy id e1 e2) (eval-env e2 (env-add id (clo id e1 env) env))]
...
;;; (define progem 
;;;     '(let [curry (lambda (f) (lambda (a) (lambda (b) (f a b))))]
...
(eval (parse '(scar (unfold 0 (lambda (x) (cons (* x x) (+ 1 x)))))))

(eval (parse' (scar (scdr (unfold 0 (lambda (x) (cons (* x x) (+ 1 x))))))))
; [0, 0, 1, 4, 9, ...]
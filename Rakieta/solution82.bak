...
(define (env-add x v env)
  (environ (cons (cons x (cons v env)) (environ-xs env))))
...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x) (eval-env (car(env-lookup x env))(cdr(env-lookup x env)))]
    ...
    [(app f e) ; <------------------------------------------------ !!!
     (let ([vf (eval-env f env)])
       (match vf [(clo x body fun-env)
                  (eval-env body (env-add x e fun-env))]))]))
...
(eval (parse'(let [y 2] (let [x y] (let [y 3] x)))))